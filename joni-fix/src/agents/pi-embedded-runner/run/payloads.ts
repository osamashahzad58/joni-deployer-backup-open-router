...
/** Path prefixes under which MEDIA: absolute paths are accepted (e.g. workspace, container home). */
const DEFAULT_MEDIA_PATH_PREFIXES = ["/home/node"];
...
export function buildEmbeddedRunPayloads(params: {
  ...
  /** Workspace dir; when set, MEDIA paths under this and /home/node are accepted for one-go image send. */
  workspaceDir?: string;
}): Array<{
  text?: string;
  mediaUrl?: string;
  ...
  const allowedPathPrefixes =
    params.workspaceDir?.trim() ?
      [params.workspaceDir.trim(), ...DEFAULT_MEDIA_PATH_PREFIXES]
    : DEFAULT_MEDIA_PATH_PREFIXES;
  const parseOpts = { allowedPathPrefixes };
  ...
  if (inlineToolResults) {
    for (const { toolName, meta } of params.toolMetas) {
      ...
      const {
        ...
        replyToTag,
        replyToCurrent,
      } = parseReplyDirectives(agg, parseOpts);
      if (cleanedText) {
        replyItems.push({
          text: cleanedText,
          media: mediaUrls,
  ...
  for (const text of answerTexts) {
    const {
      ...
      replyToTag,
      replyToCurrent,
    } = parseReplyDirectives(text, parseOpts);
