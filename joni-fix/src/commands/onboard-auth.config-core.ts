...
export function applyOpenrouterProviderConfig(cfg: OpenClawConfig): OpenClawConfig {
  ...
  const existingOpenrouter = providers.openrouter;
  const existingModels = Array.isArray(existingOpenrouter?.models) ? existingOpenrouter.models : [];
  providers.openrouter = {
    ...(typeof existingOpenrouter === "object" && existingOpenrouter !== null
      ? existingOpenrouter
      : {}),
    baseUrl: OPENROUTER_BASE_URL,
    api: "openai-completions",
    models: existingModels.length > 0 ? existingModels : [],
  };

  const defaults = { ...cfg.agents?.defaults, models };
  const imageModel = defaults.imageModel as { primary?: string; fallbacks?: string[] } | undefined;
  if (!imageModel?.primary) {
    defaults.imageModel = {
      primary: "openrouter/google/gemini-3-pro-image-preview",
      fallbacks: [
