...
function isPathUnderPrefix(resolvedPath: string, prefix: string): boolean {
  const n = prefix.replace(/\/+$/, "");
  return resolvedPath === n || resolvedPath.startsWith(n + "/");
}

function isValidMedia(
  candidate: string,
  opts?: { allowSpaces?: boolean; allowedPathPrefixes?: string[] },
) {
  if (!candidate) {
    return false;
  }
  ...
  // Relative paths: only allow safe ./ paths that do not traverse upwards.
  if (candidate.startsWith("./") && !candidate.includes("..")) {
    return true;
  }

  // Absolute or ~ paths: allow only under allowedPathPrefixes (e.g. workspace, /home/node in container).
  const prefixes = opts?.allowedPathPrefixes;
  if (prefixes?.length && (candidate.startsWith("/") || candidate.startsWith("~"))) {
    const resolved = path.resolve(
      candidate.startsWith("~") ? candidate.replace(/^~/, process.env.HOME ?? "~") : candidate,
    );
    ...
    for (const p of prefixes) {
      ...
      if (isPathUnderPrefix(resolved, norm)) {
        return true;
      }
    }
  }
...
export type SplitMediaFromOutputOptions = {
  /** If set, absolute and ~ paths under one of these prefixes are accepted (e.g. workspace, /home/node). */
...
export function splitMediaFromOutput(
  ...
  const mediaOpts = (overrides?: { allowSpaces?: boolean }) =>
    allowedPrefixes?.length
      ? { ...overrides, allowedPathPrefixes: allowedPrefixes }
      : overrides;
  ...
  for (const line of lines) {
    ...
    for (const match of matches) {
      ...
      for (const part of parts) {
        ...
        if (isValidMedia(candidate, mediaOpts(unwrapped ? { allowSpaces: true } : undefined))) {
      ...
      if (
        ...
        if (isValidMedia(fallback, mediaOpts({ allowSpaces: true }))) {
