...
export async function copyA2uiAssets({ srcDir, outDir }: { srcDir: string; outDir: string }) {
  const skipMissing =
    process.env.OPENCLAW_A2UI_SKIP_MISSING === "1" || process.env.JONI_A2UI_SKIP_MISSING === "1";
  try {
    ...
    if (skipMissing) {
      console.warn(`${message} Skipping copy (OPENCLAW_A2UI_SKIP_MISSING=1 or JONI_A2UI_SKIP_MISSING=1).`);
      return;
    }
